...
The service listens on `0.0.0.0:80` by default. The systemd unit grants `CAP_NET_BIND_SERVICE` so the non-root `tvokx` user can bind port 80. The `/upgrade` endpoint writes status to `/var/lib/tv-okx-bot/upgrade-status.json`, then schedules `/usr/bin/sudo /bin/systemctl restart tv-okx-bot.service`; the installer adds a narrow sudoers rule for that restart command.
