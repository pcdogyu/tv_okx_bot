...
- ADX-routed order records expose `source_action`, `adx`, and `market_strategy`. Current positions expose `market_strategy` and `entry_adx` only when the exchange entry fill matches the submitted tvbot order ID; unmatched or manual positions remain unlabelled.
