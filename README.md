...
- `tradingview/SMC_Order_Block_Only_Test_Strategy_v4_1_0.pine` adds confirmed ADX routing metadata to `strategy.order.alert_message`. Alerts use `intent|script=4.1.0|adx=<value>|adx_tf=<timeframe>`; ADX above 25 keeps the source direction, ADX below 20 is reversed server-side, and entries from 20 through 25 are ignored. Existing alerts without the 4.1.0 marker keep their legacy direction.
